...
//struct Stack stack;
vector <string> stack;						//构建语法树分析树的操作栈
vector <string> midcode_stack_mul;			//乘除法中间代码生成用到的操作栈		
vector <string> temp_arr_mul;				//乘除法中间代码生成暂存Tn的队列
vector <string> midcode_stack_add;			//加减法中间代码生成用到的操作栈		
vector <string> temp_arr_add;				//加减法中间代码生成暂存Tn的队列
...
map<int, string> treaddmap;		//记录三地址式
...
*/
//输出栈中信息
void stack_show(vector<string> stack, fstream &out)
{
	int i;
	out << "当前栈中元素个数:" << stack.size();
	//cout << endl;
...
void make_tree(fstream &in, fstream &out)
{
	...
	while (!in.eof())
	{
		...
		//cout << "------------------------------" << endl;
		//cout << "token.re = " << token.re << " token.ty = " << token.type << endl;
		//cout << "------------------------------" << endl;

		

		stack_show(stack, out);
		str = stack[stack.size() - 1];//stack_gettop(stack);
		...
		{	//直到栈顶元素为终结符 跳出循环 准备比较\出栈操作
			...
			else
			{
				...
				//cout << "line = " << line << endl;//
				//cout << "stackpop(" << stack_gettop(stack) << ")" << endl;//
				...
				for (iter = grammarmap[line].end() - 1; iter != grammarmap[line].begin(); iter--)
				{
					...
					if (*iter != "empty")
					{
						...
						//cout << "*iter = " << *iter << " 入栈（非空）" << endl;//
					}
				}		//将不为empty的产生式 压入栈中
				//cout << "正向为 " << cur->re << " 建立子节点: " << endl;//
				for (iter = grammarmap[line].begin() + 1; iter != grammarmap[line].end(); iter++)
				{
					...
					if (cur->mtnum == 42)
					{
						...
						//42 iteration-stmt -> while ( $41_1 expression $41_2 ) statement $41_3
					...
					if (cur->mtnum == 39)
					{
						...
						if (*iter == "selection-stmt'")
						{
							...
							strcpy_s(cur->sons[cur->bitnum]->type, "todo");
						}	//39 selection-stmt -> if ( expression $38_1 ) statement $38_2 selection-stmt' $38_3
					}		//if语句推导中需要增加两个中间语义动作
					if (cur->mtnum == 41)
					{
						if (*iter == "statement")
				...
				//cout << cur->sons[cur->bitnum]->re << endl;//<< " 子节点类型：" << cur->sons[cur->bitnum]->type << endl;//
				//cout << "cur = " << cur->sons[1]->re << endl;//
				...
				if (strcmp(cur->re, "empty") == 0)
				{
					...
					while (1)
					{
						while (!cur->brother)
						{
							...
							//cout << "(向上找到有兄弟节点的节点)cur = " << cur->re << endl;//
						...
						//cout << "(向右找到兄弟节点，且不是$开头)cur = " << cur->re << endl;//

						if (cur->re[0] != '$')
							break;
					}
				}		//应用完当前产生式 进入兄弟节点继续推导 没有兄弟节点 返回父亲节点 进入它的兄弟节点 直到兄弟节点存在
				else {
					//cout << "向下找";
				}
				//cout << " cur = " << cur->re << endl;//

				str = stack[stack.size() - 1];
				stack_show(stack, out);
			}//else
		}//while
		if (token.re != stack[stack.size() - 1] && token.type != stack[stack.size() - 1])
		{
			out << "Grammar Error2 at Line " << token.line << " Colume " << token.colume << endl;
			cout << "Grammar Error2 at Line " << token.line << " Colume " << token.colume << endl;
			exit(0);
...
{
	fstream fp;
	fp.open("14 three_add_code.txt", ios::out);

	map<int, string>::iterator iter_i_s;

	for (iter_i_s = treaddmap.begin(); iter_i_s != treaddmap.end(); iter_i_s++)
		fp << iter_i_s->first<< " (" << iter_i_s->second << ")" << endl;
...
bool fomula_13(treebit* temp)
{
	...
	treaddmap.insert({ curnum, proc + ", -, -, " + temp->sons[2]->re });
	return true;
}
...
bool fomula_39_1(treebit* temp)
{
	...
	//为真是强制跳转当前位置+2的位置
	treaddmap.insert({ curnum, "jnz, " + placelistmap[temp->sons[3]->nodenum][0] + ", -, " + ss.str()});
	curnum++;
	midcodemap.insert({ curnum,"Goto " });
	treaddmap.insert({ curnum,"j, -, -, " });
	//为假强制跳转至假出口 暂时没有求出 
	return true;
}
...
bool fomula_39_3(treebit* temp)
{
	...
	if (temp->sons[8]->info->emptyflag)
	{
		...
		treaddmap[temp->info->ifnum1].append(ss.str());
		midcodemap.erase(curnum);
		treaddmap.erase(curnum);
		curnum--;
	}		//emptyflag为1即不存在else语句 删除最后一条强制跳转语句 并且将之前空着的判断条件为假时的跳转位置填上
	else
	{
		stringstream ss;
		...
		treaddmap[temp->info->ifnum2].append(s.str());
...
bool fomula_41_1(treebit* temp)
{
	...
	temp->info->elsenum1 = curnum;		//记录else的入口地址 在41_3时填入
	return true;
}
bool fomula_41_2(treebit* temp)
{
	...
	temp = temp->father;
	temp->info->emptyflag = 0;
	temp->info->elsenum2 = curnum;		//记录else的出口地址 在41_3时填入
	return true;
}
...
bool fomula_42_2(treebit* temp)
{
	...
	treaddmap.insert({ curnum,"jnz, " + placelistmap[temp->sons[4]->nodenum][0] + ", -, " + ss.str()});
...
bool fomula_42_3(treebit* temp)
{
	...
	treaddmap[temp->info->whilenum2 + 1].append(ss.str());
	//回填假出口值
	curnum++;
	stringstream s;
	s << temp->info->whilenum1;
	midcodemap.insert({ curnum,"Goto " + s.str() });
	treaddmap.insert({ curnum,"j, -, ,- " + s.str() });
	//插入中间代码 强制跳转回42_1位置
	return true;
}
bool fomula_43(treebit* temp)
{
	...
	if (temp->sons[2]->info->emptyflag)
	{
		...
		treaddmap.insert({ curnum,"ret, -, -, -" });
	}	//无返回值
	else
	{
		curnum++;
		midcodemap.insert({ curnum,"return " + placelistmap[temp->sons[2]->nodenum][0] });
		treaddmap.insert({ curnum,"ret, -, -, " + placelistmap[temp->sons[2]->nodenum][0] });
	}	//有返回值
	return true;
}
...
bool fomula_46(treebit* temp)
{
	...
	treaddmap.insert({ curnum,":=, " + placelistmap[temp->sons[3]->nodenum][0] + ", -, " + placelistmap[temp->sons[1]->nodenum][0] });
	return true;
}
...
bool fomula_54(treebit* temp)
{
	...
	else
	{
		...
		string tempstr = placelistmap[temp->sons[2]->nodenum][0];
		char *op = strtok((char *)tempstr.c_str(), " ");
		char *arg = strtok(NULL, " ");

		midcodemap.insert({ curnum, t + " := " + placelistmap[temp->sons[1]->nodenum][0] + " " + placelistmap[temp->sons[2]->nodenum][0] });
		treaddmap.insert({ curnum, (string)op + ", " + placelistmap[temp->sons[1]->nodenum][0] + ", " + (string)arg + ", " + t});
	}
	return true;
}
...
bool fomula_63(treebit* temp)
{
	...
	{
		string t = newtemp();
		temp_arr_add.push_back(t);
		placelistmap[temp->nodenum].push_back(t);
		
		string tempstr = midcode_stack_add[midcode_stack_add.size() - 1];
		curnum++;
		midcodemap.insert({ curnum, temp_arr_add[0] + " := " + placelistmap[temp->sons[1]->nodenum][0] + " " + tempstr });
		
		char* op = strtok((char*)tempstr.c_str(), " ");
		char* arg = strtok(NULL, " ");
		treaddmap.insert({ curnum, (string)op + ", " + placelistmap[temp->sons[1]->nodenum][0] + ", " + (string)arg + ", " + temp_arr_add[0] });
		midcode_stack_add.pop_back();

		for (int i = 1; i < temp_arr_add.size(); i++) {
			tempstr = midcode_stack_add[midcode_stack_add.size() - 1];
			string tempT = temp_arr_add[i - 1];
			curnum++;
			midcodemap.insert({ curnum, temp_arr_add[i] + " := " + tempT + " " + tempstr });

			char* op = strtok((char*)tempstr.c_str(), " ");
			char* arg = strtok(NULL, " ");
			treaddmap.insert({ curnum, (string)op + ", " + tempT + ", " + *arg + ", " + temp_arr_add[i] });
			midcode_stack_add.pop_back();
		}
		temp_arr_add.clear();
	...
	/*	string tempstr = placelistmap[temp->sons[2]->nodenum][0];
		...
		midcodemap.insert({ curnum,t + " := " + placelistmap[temp->sons[1]->nodenum][0] + " " + placelistmap[temp->sons[2]->nodenum][0] });
		//treaddmap.insert({ curnum,t + " := " + placelistmap[temp->sons[1]->nodenum][0] + " " + placelistmap[temp->sons[2]->nodenum][0] });
		treaddmap.insert({ curnum, (string)op+ ", " + placelistmap[temp->sons[1]->nodenum][0] + ", " + *arg + ", " + t});
		*/
	}
	return true;
}
bool fomula_64(treebit* temp)
{
	...
	else
	{
		...
		string tempstr = placelistmap[temp->sons[3]->nodenum][0];
		char* op = strtok((char*)tempstr.c_str(), " ");
		char* arg = strtok(NULL, " ");

		midcodemap.insert({ curnum,t + " := " + placelistmap[temp->sons[2]->nodenum][0] + " " + placelistmap[temp->sons[3]->nodenum][0] });
		treaddmap.insert({ curnum, (string)op + ", " + placelistmap[temp->sons[2]->nodenum][0] + ", " + *arg + ", " + t });
	*/
	}
	midcode_stack_add.push_back(placelistmap[temp->sons[1]->nodenum][0] + " " + placelistmap[temp->sons[2]->nodenum][0]);
	return true;
}
...
bool fomula_68(treebit* temp)
{
	...
	{
		string t = newtemp();
		temp_arr_mul.push_back(t);
		placelistmap[temp->nodenum].push_back(t);

		string tempstr = midcode_stack_mul[midcode_stack_mul.size() - 1];		
		curnum++;
		midcodemap.insert({ curnum, temp_arr_mul[0] + " := " + placelistmap[temp->sons[1]->nodenum][0] + " " + tempstr });
		
		char* op = strtok((char*)tempstr.c_str(), " ");
		char* arg = strtok(NULL, " ");
		treaddmap.insert({ curnum, (string)op + ", " + placelistmap[temp->sons[1]->nodenum][0] + ", " + (string)arg + ", " + temp_arr_mul[0]});
		midcode_stack_mul.pop_back();

		for (int i = 1; i < temp_arr_mul.size(); i++) {
			tempstr = midcode_stack_mul[midcode_stack_mul.size() - 1];
			string tempT = temp_arr_mul[i - 1];
			curnum++;
			midcodemap.insert({ curnum, temp_arr_mul[i] + " := " + tempT + " " + tempstr });

			char* op = strtok((char*)tempstr.c_str(), " ");
			char* arg = strtok(NULL, " ");
			treaddmap.insert({curnum, (string)op + ", " + tempT + ", " + *arg + ", " + temp_arr_mul[i] });
			midcode_stack_mul.pop_back();
		}
		temp_arr_mul.clear();
		/*curnum++;
...
bool fomula_69(treebit* temp)
{
	...
	midcode_stack_mul.push_back(placelistmap[temp->sons[1]->nodenum][0] + " " + placelistmap[temp->sons[2]->nodenum][0]);
...
bool fomula_77(treebit* temp)
{
	...
	if (!temp->sons[3]->info->emptyflag)
		for (iter = placelistmap[temp->sons[3]->nodenum].begin(); iter != placelistmap[temp->sons[3]->nodenum].end(); iter++)
		{
			...
			treaddmap.insert({ curnum, param + ", -, -, " + *iter});
		}
	curnum++;
	midcodemap.insert({ curnum, t + " := " + call + temp->sons[1]->re });
	treaddmap.insert({ curnum, call + ", " + temp->sons[1]->re + ", -" + ", " + t });
...
void deepsearch(treebit *temp,fstream &out)
{
	...
	if (strstr(temp->type, "derivation") != 0)//非终结符
	{
